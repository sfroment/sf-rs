...
use crate::{
	...
	websocket::{WebSocketConnection, WsSenderState},
};

// Add From implementation for sf_peer_id::Error to JsError
impl From<sf_peer_id::Error> for JsError {
	fn from(e: sf_peer_id::Error) -> Self {
		JsError::new(&format!("PeerID error: {}", e))
	}
}
...
impl Client {
	pub fn new() -> ClientResult<Rc<Self>> {
		let peer_id = PeerID::random().map_err(|e| JsError::new(&format!("Failed to generate peer ID: {e}")))?;
		...
		PrometheusBuilder::new()
			...
			.map_err(|e| JsError::new(&format!("Failed to configure Prometheus push gateway: {e}")))?
			.idle_timeout(
	...
	pub fn list_peers(self: &Rc<Self>) -> Vec<String> {
		self.borrow_pm()
			.map(|pm| pm.get_known_peer_ids().iter().map(|p| p.to_string()).collect())
			.unwrap_or_default()
	}
	...
	fn borrow_ws(&self) -> ClientResult<Ref<WebSocketConnection>> {
		self.ws
			...
			.and_then(|guard| {
				Ref::filter_map(guard, |opt| opt.as_ref()).map_err(|_| JsError::new("WebSocket is not initialized"))
			})
	}
	...
	async fn handle_peer_event(self: &Rc<Self>, from_peer_id: PeerID, event: PeerEvent) -> ClientResult<()> {
		...
		match event {
			...
			PeerEvent::WebRtcOffer {
				session_description, ..
			} => self.handle_web_rtc_offer(&from_peer_id, &session_description).await?,
			PeerEvent::WebRtcCandidate { candidate, .. } => {
				self.handle_web_rtc_candidate(&from_peer_id, &candidate).await?
			}
		}
		Ok(())
	}

	async fn handle_web_rtc_candidate(&self, peer_id: &PeerID, candidate: &IceCandidate) -> Result<(), JsError> {
		info!(%peer_id, %self.peer_id, "Received WebRtcCandidate");
		let peer = self
			.borrow_pm()?
			.get_peer(peer_id)
			.cloned()
			.ok_or_else(|| JsError::new(&format!("Peer not found to handle candidate: {peer_id}")))?;
	...
	pub async fn send_message_to_peer(&self, peer_id: PeerID, message: String) -> Result<(), JsError> {
		let peer = {
			self.borrow_pm()?
				.get_peer(&peer_id)
				.cloned()
				.ok_or_else(|| JsError::new(&format!("Peer not found for sending message: {peer_id}")))?
		};
