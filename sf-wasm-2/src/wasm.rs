...
pub struct Client {
    ...
    on_new_peer_callbacks: RefCell<Vec<js_sys::Function>>,
...
impl Client {
    pub fn new() -> Result<Rc<Self>, JsValue> {
        ...
        Ok(Rc::new(Self {
            ...
            on_new_peer_callbacks: RefCell::new(Vec::new()),
    ...
    pub fn on_new_peer(self: &mut Rc<Self>, callback: js_sys::Function) -> usize {
        let mut callbacks = self.on_new_peer_callbacks.borrow_mut();
        let id = callbacks.len();
        callbacks.push(callback);
        id
    }

    pub fn remove_on_new_peer(self: &mut Rc<Self>, id: usize) {
        let mut callbacks = self.on_new_peer_callbacks.borrow_mut();
        callbacks.remove(id);
    }
                            ...
                            match peer_request {
                                PeerRequest::Forward { data, .. } => match data {
                                    PeerEvent::NewPeer { peer_id } => {
                                        client.peers.borrow_mut().push(peer_id);
                                        for callback in client.on_new_peer_callbacks.borrow().iter()
                                        {
                                            let this = JsValue::NULL;
                                            let peer_id_js =
                                                serde_wasm_bindgen::to_value(&peer_id.to_string())
                                                    .unwrap();
                                ...
                                _ => {
                                    log!("unhandled event type: {:?}", peer_request.clone());
                                }
                            }
                        }
                    }
...
impl ClientWrapper {
    ...
    pub fn list_peers(&self) -> Vec<String> {
        log!("list_peers: {:?}", self.client.list_peers().len());
        let peers = self.client.list_peers();
        let peers_str: Vec<String> = peers.iter().map(|p| p.to_string()).collect();
        log!("peers_str: {:?}", peers_str.clone());
        peers_str
    }
    ...
    pub fn on_new_peer(&mut self, callback: js_sys::Function) -> usize {
        self.client.on_new_peer(callback)
    }

    pub fn remove_on_new_peer(&mut self, id: usize) {
        self.client.remove_on_new_peer(id);
    }
}
